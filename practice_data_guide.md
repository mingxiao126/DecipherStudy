...
### 5. 换行符与非法控制字符 (\n)
**绝对禁止在 JSON 字符串值中直接输入真实换行符 (Raw Newline, 0x0A)**。如果你需要在题目、分析、表格中换行，必须使用严格的转义字符 `\n`（在 JSON 代码中呈现为 `\\n`）。直接输出真实换行符会导致解析器报 `Bad control character in string literal` 错误。
- ❌ **错误 (出现真实的换行)**: `"content": "第一行文字
第二行文字"`
- ✅ **正确 (使用转义)**: `"content": "第一行文字\n第二行文字"`
